...
from G14Utils import registry_add, registry_remove, registry_check
...
def activate_powerswitching():
    ...
    if power_thread is None:
        ...
        notify("Power switching has been activated.")
    if (
        config["default_gaming_plan"] is not None
        and config["default_gaming_plan_games"] is not None
    ):
...
class gaming_thread_impl(threading.Thread):
    ...
    def run(
        self,
    ):  # Checks if user specified games/programs are running, and sw`itches to user defined plan, then switches back once closed
...
def get_current():
    ...
    plan_idx = next(
        i for i, e in enumerate(config["plans"]) if e["name"] == current_plan
    )
        ...
        map(
            lambda winplan: (
                MenuItem(
                    ...
                    checked=lambda menu_itm: active_plan_map[winplan[1]],
                )
            ),
            windows_plans,
        )
...
def startup_checks():
    global default_ac_plan, auto_power_switch, registry_key_loc, G14dir
    # Only enable auto_power_switch on boot if default power plans are enabled (not set to null):
    if (
        default_ac_plan is not None
        and default_dc_plan is not None
        and config["power_switch_enabled"] is True
    ):
        auto_power_switch = True
    else:
        auto_power_switch = False
...
def reload_config():
    ...
    dpp_GUID, app_GUID = get_power_plans()
    ...
    main_cmds = RunCommands(
        config, G14dir, app_GUID, dpp_GUID, notify, windows_plans, active_plan_map
    )

    if auto_power_switch and config["power_switch_enabled"]:
        power_thread = power_check_thread()
        power_thread.start()
